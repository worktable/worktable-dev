...
function localStateFailure(error: unknown): boolean {
  return [
    "ENOSPC",
    "EDQUOT",
    "EROFS",
...
export class WorktableConnector {
  ...
  async run(stopSignal: AbortSignal): Promise<void> {
    ...
    const signal = AbortSignal.any([stopSignal, failed.signal])
    let fatalError: unknown
    const stopOnLocalFailure = (error: unknown) => {
      if (!localStateFailure(error) || failed.signal.aborted) return
      fatalError = error
      failed.abort(error)
    }
    ...
    while (!signal.aborted) {
      try {
        if (!connected) {
          await this.#client.participants(signal)
        ...
        const running = this.#handle(delivery, signal, stopOnLocalFailure)
          .catch((error) => {
  ...
  async #handle(
    delivery: ClaimedWorktableDelivery,
    signal?: AbortSignal,
    stopOnLocalFailure?: (error: unknown) => void
  ): Promise<void> {
    ...
    try {
      ...
      if (terminalAfterDispatch) {
        try {
          ...
          stopOnLocalFailure?.(commitError)
