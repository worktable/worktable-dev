...
function DocEditorPage({
  ...
  const recoverOfflineEdits = useCallback(async () => {
    ...
    try {
      ...
      const { blocksFromCollaborationDoc } = await import(
  ...
  const handleConvertToMarkdown = useCallback(async () => {
    ...
    try {
      ...
      toast.error("Couldn’t save Markdown. Try again.")
    } finally {
  ...
  return (
    <BlockNoteDocPage
      ...
      onMarkReviewed={handleMarkReviewed}
      onConvertToMarkdown={
        !docData.archived && docData.markdownCompatible
...
function BlockNoteDocPage({
  ...
  useInferredReview(doc, Boolean(provider && collabDoc), onInferredReview)

  if (!provider || !collabDoc) {
    return <EditorSkeleton />
  }
