...
export async function startWebHarness(
  ...
  try {
    ...
    const web = await startOnAllocatedPort(
      (port) =>
        startProcess(
          ...
          join(repoRoot, "apps/web"),
          {
            ...
            WORKTABLE_API_PORT: String(apiPort),
            WORKTABLE_API_URL: apiUrl,
            WORKTABLE_WEB_PORT: String(port),
          },
          children,
          processOutput
        ),
      // Keep process readiness separate from the browser journey's cold SPA
      // transform while still proving that this exact Vite child serves HTTP.
      (port) => `http://127.0.0.1:${port}/favicon.svg`,
      processOutput,
      { readyOutput: /ready in \d+ ms/i }
    )
    const webUrl = new URL(web.url).origin
