...
export class McpWorktableClient implements WorktableClient {
  readonly #createTransport: () => StreamableHTTPClientTransport
  #session?: { client: Client; transport: StreamableHTTPClientTransport }
  ...
  async #ensureConnected(signal?: AbortSignal): Promise<Client> {
    if (this.#closed) throw new Error("Worktable client is closed")
    ...
    this.#connecting = (async () => {
      try {
        await session.client.connect(session.transport, { signal })
        if (this.#closed) throw new Error("Worktable client is closed")
        this.#connected = true
        return session.client
      } catch (error) {
        // MCP transports cannot be started twice. Dispose the failed attempt
        // so the connector's bounded retry can establish a fresh connection.
  ...
  async close(): Promise<void> {
    ...
    // Close even during initialization, before #connected becomes true.
    if (session) {
  ...
  async participants(signal?: AbortSignal): Promise<WorktableParticipant[]> {
